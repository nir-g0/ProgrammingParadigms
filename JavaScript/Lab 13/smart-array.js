  ...
  return new Proxy(args, {
    get: function (target, prop) {
      if (prop.match(RANGE_PAT)) {
        ...
        // raise an exception.
        let lenOfArgs = arguments[0].length
        let indx = parseInt(prop.match(FROM_END_PAT))
        if (indx < -lenOfArgs) {
          throw 'index position is negative'
        }

        return Reflect.get(arguments[0], lenOfArgs + indx, arguments[2])
      } else {
    ...
    set: function (target, prop, newVal) {
      if (prop.match(NUM_PAT)) {
        ...
        return Reflect.setet(arguments[0], lenOfArgs + indx, arguments[2])
      } else {
